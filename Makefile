...
FILES = Preface Basics Induction Lists
...
gen_md:
	$(foreach file, $(FILES),v2md $(addprefix en_v/,$(file)).v -o $(addprefix en_md/,$(file)).md;)

clean:
	rm -r $(BUILD)

md: $(BUILD)/md/$(BOOKNAME).pdf
...
$(BUILD)/pdf/$(BOOKNAME).pdf: $(TITLE) $(addsuffix .md,$(addprefix en_md/, $(FILES)))
	mkdir -p $(BUILD)/pdf
